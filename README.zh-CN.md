...
- **29 种语言**的 `ChangeLog`，**16 个 App Store 地区**的元数据——保持统一、文档化的顺序。
- **默认安全** — 所有远端变更均先 dry-run 预览并要求显式确认；未经要求绝不使用 `--allow-deletes`。
- **超时恢复** — 从容处理 `asc` 超时，不假设部分回滚。
- **源语言驱动翻译** — 简体中文为规范源语言，英文为主要翻译目标。
...
3. 将原始输出打磨为面向用户的发布说明（源语言：`zh-Hans`）。
4. 翻译为全部 29 种 `ChangeLog` 语言。
5. 在 `ChangeLog` 顶部插入新版本块。
6. 创建 `AppStoreConnect/metadata/version/<版本>/`（从上一版本复制），仅更新全部 16 个地区的 `whatsNew`。
